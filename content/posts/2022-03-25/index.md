...
```
The `AF_INET` singifies the internet Protocol family for the socket. The `SOCK_RAW` singifies that it will be in in “raw” mode. The `IPPROTO_TCP` means that it will be sending TCP packets . With the `setsockopt()`, the kernel will not generate an IP header since it is being provided by my program.
...
## TCP SYN Scan
SYN scan is the deafault and the most popular scan option, since it can performed quickly, scanning thousandds of ports per seconds. SYN scan is relatively unobtrusive and stealthy because it neve completes the TCP connections.
...
In the case, when multiuple `[SYN]` packets are sent and there are no reponse, these ports will be classify as filtered.
